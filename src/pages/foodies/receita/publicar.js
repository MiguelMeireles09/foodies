...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const filteredIngredientes = formData.ingredientes.filter(ingrediente => ingrediente.trim() !== "");
    const filteredQuantidades = formData.quantidades.filter((quantidade, index) => formData.ingredientes[index].trim() !== "");
    const filteredModoPreparo = formData.modoPreparo.filter(step => step.trim() !== "");
  ...
  <main className="container max-w-4xl mx-auto p-8 bg-white shadow rounded-lg">
    <h2 className="text-2xl font-bold text-center text-gray-800 mb-8">Postar Receita</h2>
...
{formData.ingredientes.map((ingrediente, index) => (
  <div key={`ingrediente-${index}`} className="flex flex-wrap -mx-2 mb-4">
...
<button type="submit" className="mt-6 w-full bg-verde hover:bg-green-700 text-white font-bold py-3 px-4 rounded-lg transition duration-300">
  Postar Receita
</button>
        </form>
      </main>
